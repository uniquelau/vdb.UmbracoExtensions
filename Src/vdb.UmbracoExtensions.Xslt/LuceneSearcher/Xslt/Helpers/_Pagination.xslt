...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:umb="urn:umbraco.library" xmlns:str="urn:Exslt.ExsltStrings" xmlns:make="urn:schemas-microsoft-com:xslt" version="1.0" exclude-result-prefixes="umb str make">
	...
	<!-- Config constants -->
	<xsl:variable name="pagerParam" select="'p'"/>
	<!-- Name of QueryString parameter for 'page' -->
	...
	<xsl:variable name="pageLinksBeside" select="'4'"/>
	<!-- Number of pagination links to show before and after the current page -->
	...
	<xsl:template name="PaginateSelection">
		...
		<xsl:variable name="startIndex" select="$perPage * ($page - 1) + 1"/>
		<!-- First item on this page -->
		<xsl:variable name="endIndex" select="$page * $perPage"/>
	...
	<xsl:template name="RenderPager">
		...
		<ul class="pager">
			...
			<li class="prev">
				<xsl:choose>
					...
					<xsl:when test="$page = 2">
						<a href="{$query}" rel="prev start">
							<xsl:value-of select="$prevPage"/>
						</a>
					</xsl:when>
					<xsl:otherwise>
						<a href="{$query}{$sep}{$pagerParam}={$page - 1}" rel="prev">
							<xsl:value-of select="$prevPage"/>
						</a>
					</xsl:otherwise>
				</xsl:choose>
			...
			<xsl:if test="$needToRenderGaps and ($page - $pageLinksBeside &gt; 4)">
				<li>
					<a href="{$query}" rel="start">1</a>
				</li>
				<li>
					<a href="{$query}{$sep}{$pagerParam}=2">2</a>
				</li>
				<li class="gap">...</li>
			</xsl:if>
			...
			<xsl:if test="$needToRenderGaps and ($page + $pageLinksBeside &lt; $lastPageNum - 3)">
				<li class="gap">...</li>
				<li>
					<a href="{$query}{$sep}{$pagerParam}={$lastPageNum - 1}">
						<xsl:value-of select="$lastPageNum - 1"/>
					</a>
				</li>
				<li>
					<a href="{$query}{$sep}{$pagerParam}={$lastPageNum}">
						<xsl:value-of select="$lastPageNum"/>
					</a>
				</li>
			</xsl:if>
			...
			<li class="next">
				<xsl:choose>
					...
					<xsl:otherwise>
						<a href="{$query}{$sep}{$pagerParam}={$page + 1}" rel="next">
							<xsl:value-of select="$nextPage"/>
						</a>
					</xsl:otherwise>
				</xsl:choose>
